...
def ascend_batch_transfer_weights(
    ...
    for name, param in state_dict.items():
        if name.endswith("expert_map"):
            continue
        ptr = param.data_ptr()
        if ptr not in all_params_ptrs and ptr:
    ...
    def handle_sub_module(submodule, submodule_name):
        for attr_name, attr_value in submodule.__dict__.items():
            if isinstance(attr_value, torch.Tensor):
                ...
                if data_ptr not in all_params_ptrs and data_ptr:
    ...
    for module_name, module in model.named_modules():
        if (expert_map := getattr(module, "ascend_expert_map", None)) is not None:
...
class AscendElasticEPScalingExecutor(ElasticEPScalingExecutor):
    def __init__(self, worker):
        ...
        self._eplb_manager = ElasticEplbManager(worker)
    ...
    def switch_and_prepare(self) -> None:
        ...
        num_logical_experts = moe_modules[0].moe_config.num_logical_experts
        assert all(module.moe_config.num_local_experts == num_local_experts for module in moe_modules), (
            "All MoE modules must have the same number of experts"
        )
        for module in moe_modules:
            module.moe_config.num_experts = num_local_experts * new_ep_size
            module.routed_experts.global_redundant_expert_num = num_local_experts * new_ep_size - num_logical_experts
    ...
    def prepare_new_worker(self) -> None:
        # There is nothing to do in vLLM Ascend.
        pass
    ...
    def _setup_moe_comm_and_quant_method(self) -> None:
        moe_modules = [module for module in self.worker.get_model().modules() if isinstance(module, MoERunner)]
        for module in moe_modules:
            with set_current_vllm_config(self.worker.vllm_config):
                setup_moe_comm_and_quant_method(module)