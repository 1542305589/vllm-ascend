...
class ElasticEplbManager:
    ...
    def init_model(self):
        self.model = self.worker.get_model()
    ...
    def get_expert_maps(self):
        ...
        for layer_id in range(self.num_moe_layers):
            ...
            ].mlp.experts.routed_experts.global_expert_map.cpu()
    ...
    def eplb(self, old_ep_size, new_ep_size):
        ...
        rank_id = get_comm_group().rank_in_group

        expert_map = []
        for layer_id in range(self.num_moe_layers):
            expert_map.append(
                self.model.model.layers[self.num_dense_layers + layer_id].mlp.experts.routed_experts.ascend_expert_map
            )
        expert_map = torch.stack(expert_map).unsqueeze(1).npu()
        expert_maps = get_comm_group().all_gather(expert_map, dim=1).cpu()
        for layer_id in range(self.num_moe_layers):
            # Scale Up
            ...
            self.model.model.layers[
                self.num_dense_layers + layer_id
            ].mlp.experts.routed_experts.global_expert_map = expert_maps_this_layer.cpu()
            self.model.model.layers[self.num_dense_layers + layer_id].mlp.experts.routed_experts.log2phy.copy_(
