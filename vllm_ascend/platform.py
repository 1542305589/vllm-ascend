...
class NPUPlatform(Platform):
    ...
    def check_and_update_config(cls, vllm_config: VllmConfig) -> None:
        ...
        if parallel_config.enable_elastic_ep:
            cls.allocate_platform_elastic_ep_ports(parallel_config)
    ...
    def allocate_platform_elastic_ep_ports(parallel_config, dp_size: int = None):
        ...
        if not hasattr(parallel_config, "_platform_stateless_ports"):
            parallel_config._platform_stateless_ports = dict()

        assert isinstance(parallel_config._platform_stateless_ports, dict)

        ascend_config = get_ascend_config()
        global_tp_size = parallel_config.tensor_parallel_size
        global_dp_size = dp_size or parallel_config.data_parallel_size
        global_pp_size = parallel_config.pipeline_parallel_size
        world_size = global_tp_size * global_dp_size * global_pp_size
        ...
        parallel_config._platform_stateless_ports["mc2"] = [
            [all_ports.pop() for _ in range(3)] for _ in range(len(group_ranks))
        ]
        if ascend_config.eplb_config.dynamic_eplb:
            parallel_config._platform_stateless_ports["dynamic_eplb"] = [
                [all_ports.pop() for _ in range(3)] for _ in range(len(group_ranks))
            ]
        if ascend_config.multistream_overlap_gate:
            parallel_config._platform_stateless_ports["fc3_quant_x"] = [
                [all_ports.pop() for _ in range(3)] for _ in range(len(group_ranks))
            ]
